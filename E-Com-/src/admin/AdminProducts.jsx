...
export default function AdminProducts() {
  ...
  // Product form
  const [productForm, setProductForm] = useState({ name: "", price: "", image: "", categoryId: "" });
  ...
  const toggleTrending = async (productId) => {
    await fetch(`${API}/products/${productId}/trending`, { method: "PUT", headers: { Authorization: token ? `Bearer ${token}` : "" } });
    loadTrending();
    loadProducts();
  };

  // HERO
  const [heroForm, setHeroForm] = useState({ type: "image", src: "", title: "", buttonText: "", link: "" });
  ...
  const deleteHero = async (id) => {
    ...
    await fetch(`${API}/hero/${id}`, { method: "DELETE", headers: { Authorization: token ? `Bearer ${token}` : "" } });
    loadHero();
  };
    ...
    <div className="admin-products-page">
      ...
      {activeTab === "categories" && (
        <div className="category-section">
          ...
          <ul className="category-list">
            {categories.map((c) => (
              <li key={c.id}>{c.name}<button onClick={() => deleteCategory(c.id)}>❌</button></li>
            ))}
          </ul>
        </div>
      )}
        ...
        <div className="trending-grid">
          {products.map((p) => (
            <div key={p.id} className={`trend-card ${trending.find((t) => t.id === p.id) ? "active" : ""}`}>
              <img src={p.image} alt={p.name} />
              <h4>{p.name}</h4>
              <button onClick={() => toggleTrending(p.id)}>{trending.find((t) => t.id === p.id) ? "Remove" : "Make Trending"}</button>
            </div>
          ))}
        </div>
      )}
      ...
      {activeTab === "hero" && (
        <div className="hero-section">
          ...
          <input name="src" placeholder="Image / Video URL" value={heroForm.src} onChange={handleHeroChange} />
          <input name="title" placeholder="Title" value={heroForm.title} onChange={handleHeroChange} />
          <input name="buttonText" placeholder="Button Text" value={heroForm.buttonText} onChange={handleHeroChange} />
          <input name="link" placeholder="Redirect Link" value={heroForm.link} onChange={handleHeroChange} />
          ...
          <div className="hero-list">
            {heroBanners.map((b) => (
              <div key={b.id} className="hero-card">
                {b.type === "image" ? <img src={b.src} alt="banner" /> : <video src={b.src} controls />}
